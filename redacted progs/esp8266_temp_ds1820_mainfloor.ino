...
#define mqtt_clientname "ESP8266Client_1"
#define mqtt_user ""
#define mqtt_password ""

#define temperature_topic "mainfloor/temperature"
#define REPORT_INTERVAL 20 // in sec
...
float Calibration_offset = 2.2; // cal from RTD
...
void loop() {
  long now = millis();
  if (now - lastMsg > (REPORT_INTERVAL * 1000)) { // only check temperature every 20 seconds
    lastMsg = now;
    if (now - timeout > 300000) { 
      snapshot = true;  // publish reading every 5 min whether it changes or not
      timeout = now;
    }
  ...
  client.loop();
  float newTemp = 0;
  for (int i=1;i<=4;i++) { // average 40 readings to smooth out jitter
    readTemperature();
    delay(10);
    newTemp += Temperature;
   }
